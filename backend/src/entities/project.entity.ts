...
export class ProjectEntity {
  ...
  @Column({ type: 'text', nullable: true }) students_json!: string | null; // JSON string
  @Column({ type: 'text', default: '' }) description!: string;
  
  // 新增字段
  @Column({ type: 'text', nullable: true }) team_members_json!: string | null; // 团队成员JSON数组，第一个为队长
  @Column({ type: 'text', nullable: true }) paper_url!: string | null; // 论文文件URL
  @Column({ type: 'text', nullable: true }) paper_filename!: string | null; // 论文原始文件名
  @Column({ type: 'text', nullable: true }) video_url!: string | null; // 视频文件URL
